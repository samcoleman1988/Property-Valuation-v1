...
def calculate_valuation(
    ...
    if base_value <= 0:
        ...
        result.investment_tagline = "Valuation failed - no usable comparable prices"
        result.valuation_status = "Insufficient evidence"
        result.sufficient_evidence = False
        return result
    ...
    # Step 9: Offer strategy (only if valuation is at least "Usable with caution")
    if result.valuation_status in ("Reliable", "Usable with caution"):
        _calculate_offer_strategy(result)
...
def _calculate_offer_strategy(result: ValuationResult):
    cfg = get_config()
    ...
    balanced = result.fair_value_balanced
    conservative = result.fair_value_conservative if result.fair_value_conservative > 0 else balanced * 0.90
    ...
    result.suggested_initial_offer = round(initial_base * (1 - buffer_pct), -3)

    result.max_sensible_offer = round(balanced, -3)
    result.walk_away_price = round(balanced * (1 + cfg.offer_strategy.walk_away_ceiling_pct), -3)
    ...
    if gap_pct > 15:
        parts.append(
            f"The asking price is {gap_pct:+.0f}% above estimated fair value. "
        ...
        parts.append(
            f"The asking price is {gap_pct:+.0f}% above fair value. "
            f"Open at {format_currency(result.suggested_initial_offer)} and "
            f"do not exceed {format_currency(result.walk_away_price)}."
        )
    elif gap_pct > -5:
        parts.append(
            f"The asking price is within {abs(gap_pct):.0f}% of fair value. "
            f"Fairly priced but still negotiate. "
            f"Open at {format_currency(result.suggested_initial_offer)}."
        )
    else:
        parts.append(
            f"The asking price is {abs(gap_pct):.0f}% below estimated fair value. "
            f"Potentially undervalued - investigate why before offering. "
            f"Open at {format_currency(result.suggested_initial_offer)}."
        )

    result.negotiation_reasoning = " ".join(parts)
...
def _generate_tagline(result: ValuationResult) -> str:
    if result.valuation_status == "Insufficient evidence":
        return "Insufficient evidence - manual research required"

    gap = result.asking_vs_fair_gap_pct
    prefix = ""
    if result.valuation_status == "Weak evidence":
        prefix = "Tentative: "
        if gap > 15:
            return f"{prefix}Possibly overpriced but evidence is weak - verify manually"
        elif gap < -10:
            return f"{prefix}Possibly undervalued but evidence is weak - verify manually"
        return f"{prefix}Weak evidence - treat valuation as rough guide only"

    if gap <= -10:
        return f"{prefix}Undervalued - investigate quickly"
    if gap < -5:
        return f"{prefix}Slightly undervalued - promising"
    if -5 <= gap <= 5:
        return f"{prefix}Fairly priced - negotiate for margin"
    if 5 < gap <= 15:
        return f"{prefix}Slightly above fair value - negotiate carefully"
    if gap > 15:
        return f"{prefix}Overpriced - negotiate hard or walk away"
