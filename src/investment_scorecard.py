...
def calculate_scorecard(
    ...
    # 1. Fair Value
    _score_fair_value(sc.fair_value, valuation, cfg)

    # 2. Negotiation Opportunity
    _score_negotiation(sc.negotiation_opportunity, valuation, cfg)
    ...
    # 8. Investment Risk
    _score_risk(sc.investment_risk, valuation, planning, btl)
    ...
    _collect_risks_and_opportunities(sc, valuation, planning, btl, location)

    # Verdict and recommendation
    sc.verdict = _generate_verdict(sc, mode)
    sc.recommendation = _generate_recommendation(sc, valuation, mode)
    sc.investment_tagline = valuation.investment_tagline

    return sc


def _score_fair_value(dim: DimensionScore, val: ValuationResult, cfg):
    """How well-priced is the property relative to evidence?"""
    gap = val.asking_vs_fair_gap_pct
    ...
    dim.key_drivers.append(f"Confidence: {val.confidence_label}")
    if val.comparables_used > 0:
        dim.key_drivers.append(f"{val.comparables_used} comparables used")


def _score_negotiation(dim: DimensionScore, val: ValuationResult, cfg):
    ...
    if val.suggested_initial_offer > 0:
        dim.key_drivers.append(f"Suggested opening: {format_currency(val.suggested_initial_offer)}")
    if val.walk_away_price > 0:
        dim.key_drivers.append(f"Walk-away price: {format_currency(val.walk_away_price)}")
...
def _score_risk(dim: DimensionScore, val: ValuationResult, planning: dict, btl: dict):
    ...
    if val.asking_vs_fair_gap_pct > 15:
        risk_score -= 2
        dim.key_drivers.append(f"Asking price {val.asking_vs_fair_gap_pct:.0f}% above fair value")
    elif val.asking_vs_fair_gap_pct > 10:
        risk_score -= 1
...
def _collect_risks_and_opportunities(
    ...
    if val.asking_vs_fair_gap_pct > 10:
...
def _generate_recommendation(
    ...
    if gap > 15:
        parts.append(
            ...
            f"Do not offer more than {format_currency(val.fair_value_balanced)} "
            f"without exceptional justification."
        )
    elif gap > 5:
        parts.append(
            f"The asking price is above estimated fair value. "
            f"Open at {format_currency(val.suggested_initial_offer)} "
            f"and do not exceed {format_currency(val.walk_away_price)}."
        )
    elif gap > -5:
        parts.append(
            f"Fairly priced. Open negotiations at "
            f"{format_currency(val.suggested_initial_offer)}."
        )
    else:
        parts.append(
