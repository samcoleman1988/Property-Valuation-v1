"""Diagnose why Willowbank Road's Direct Evidence group is now empty."""
import sys, os
...
print(f"Subject street (normalised): {subject_street}")
print(f"MAX_DIRECT_AGE_DAYS: {MAX_DIRECT_AGE_DAYS}")
...
for c, is_same, reason, prox_level in candidates:
    ...
    print(f"  {c.address[:50]:50s} | type={c.property_type_code or '?'} ({rel}) | "
          ...
          f"price={format_currency(c.price)} | same_street={is_same} prox={prox_level} | {reason}")
