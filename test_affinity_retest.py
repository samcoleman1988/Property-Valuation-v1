"""Re-test 5 properties after Development affinity fix (stopwords + threshold + unknown-type).

Reports before/after for each: comps, Dev valuation, Evidence Status, final V2.
"""
import sys, os
...
PROPERTIES = [
    ...
    {"label": "1. Ruttle Close, Cholsey (semi, 3bed, OX10)",
     "listing": PropertyListing(
         ...
         overrides_applied=["Street: Ruttle Close"])},
    ...
    {"label": "4. Ingestre Road, Prenton (flat, 2bed, CH43)",
     "listing": PropertyListing(
         ...
         overrides_applied=["Street: Ingestre Road"])},
]
...
for t in PROPERTIES:
    ...
    try:
        ...
        print(f"  V2 (after fix): {format_currency(v2_val)} | {v2.final.confidence_label} ({v2.final.confidence_score})", flush=True)
...
print(f"\n{'#' * 80}", flush=True)
print(f"  RETEST COMPLETE", flush=True)
print(f"{'#' * 80}", flush=True)
