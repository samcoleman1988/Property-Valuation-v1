...
# of the engine.
MODEL_VERSION = "v2-evidence-status-fallback-guard-real-hpi-cr1-h0-lm-type-weighting"
MODEL_VERSION_DATE = "2026-07-17"
...
class FinalValuation:
    ...
    # --- Verdict — mirrors recommendation.investment_tagline; kept as a
    # flat field for existing consumers (PDF, property_db). ---
...
def build_local_market_evidence_group(
    ...
    - compatible property type under the broad V1 gate (unchanged — this
      function still admits the same comparables it always did)
    - same postcode sector as subject
    - sold within 5 years
    - has price data
    - not an extreme price outlier (IQR fence)

    Unlike admission, VALUATION WEIGHT now applies property_type_weight()
    — the same shared type-compatibility multiplier Direct and Estate
    Evidence already use — so an admitted comp whose type is only broadly
    (not strictly) compatible with the subject no longer contributes at
    full weight. See Stage 2 forensic audit
    (validation_baselines/forensic_reports/stage2_local_market_type_weighting_audit.md)
    for why this was added: this group previously had no type-based
    discount at all, letting e.g. Detached properties dominate a
    Semi-Detached subject's local market valuation.
    """
    group = EvidenceGroup(
        name="Local Market Evidence",
    ...
    # Type diagnostics — classified with the SAME shared scheme Direct and
    # Estate Evidence use (is_property_type_compatible()), not a Local
    # Market-specific one. The admission gate above (Gate 3) still uses the
    # broader V1 _is_type_compatible() check, so a comp can be admitted here
    # while being classified "incompatible" under this stricter shared
    # scheme — it is not excluded (retrieval/admission is unchanged), but is
    # now honestly counted rather than folded into "compatible", and its
    # valuation weight below reflects that classification via
    # property_type_weight(). type_excluded_count is not populated here:
    # Local Market has no separate outlier-exclusion step for type reasons
    # (unlike Direct/Estate's fallback-admission outlier check) — every comp
    # that passes Gate 3 remains in the group, only reweighted/reclassified.
    for c in candidates:
        rel = is_property_type_compatible(subject_code, c.property_type_code)
        if rel == "exact":
            ...
            group.unknown_type_count += 1
        else:  # "incompatible"
            group.type_incompatible_fallback_count += 1

    prices = np.array([c.adjusted_price for c in candidates])
    group.median_value = round(float(np.median(prices)), -2)

    # Weighted mean: tier * recency * size_similarity * property-type
    # compatibility. property_type_weight() is the same shared helper
    # already used by Direct and Estate Evidence — not a second
    # compatibility matrix (see Stage 2 forensic audit, which found this
    # multiplier was previously entirely absent from Local Market's
    # weighting, unlike everywhere else it's used in this file).
    weights = []
    for c in candidates:
        w = _local_comp_weight(c) * property_type_weight(subject_code, c.property_type_code)
        if subject_sqm > 0 and c.floor_area_sqm > 0:
            w *= _size_similarity_weight(c.floor_area_sqm, subject_sqm)
        weights.append(w)
    weights = np.array(weights, dtype=float)
    ...
    if subject_sqm > 0:
        ...
        if sqm_comps:
            ...
            group.median_price_per_sqm = round(float(np.median(psm_values)), 0)
            sqm_weights = np.array([
                _local_comp_weight(c) * property_type_weight(subject_code, c.property_type_code)
                * _size_similarity_weight(c.floor_area_sqm, subject_sqm)
                for c in sqm_comps
            ], dtype=float)
            sqm_w_sum = sqm_weights.sum()
    ...
    # Evidence quality and status — both functions are shared with Direct
    # and Estate Evidence and already read type_exact_count/
    # type_compatible_count/type_incompatible_fallback_count generically;
    # no new thresholds are needed here. Previously Local Market always
    # populated type_incompatible_fallback_count as 0 (it had no fallback
    # concept), so these shared functions' type-composition checks were
    # structurally blind to Local Market's actual type mix — that blind
    # spot is what this change fixes, by feeding them the same honest
    # counts Direct/Estate already provide, not by changing the functions
    # themselves.
    group.evidence_quality = _calculate_evidence_quality(group)
    group.evidence_status, group.evidence_status_reason = _calculate_evidence_status(group)
...
def _assess_local_quality(
    ...
    # Factual, non-alarmist note on type composition (Stage 2 forensic
    # finding) — only when non-exact types make up a substantial share,
    # matching the >25% threshold used in that audit's own screening.
    non_exact = n - group.type_exact_count
    if n > 0 and non_exact / n > 0.25:
        group.weaknesses.append(
            "Local market evidence includes a mixture of property types; "
            "non-exact types were downweighted."
        )
...
def build_area_market_evidence_group(
    ...
    When implemented, follow Local Market Evidence's pattern for type
    weighting: multiply each comp's tier/recency weight by
    property_type_weight(subject_code, comp.property_type_code) — the same
    shared helper Direct, Estate, and (as of this change) Local Market all
    use — and populate type_exact_count/type_compatible_count/
    unknown_type_count/type_incompatible_fallback_count via
    is_property_type_compatible(), not a bespoke classification.
    """
...
def blend_evidence(
    ...
    if not active:
        ...
        final.recommendation = build_recommendation(
            fair_value_balanced=0, fair_value_conservative=0, asking_price=asking,
            asking_vs_fair_gap_pct=0, valuation_status=final.valuation_status,
            sufficient_evidence=False, source_engine="V2",
        )
        final.investment_tagline = final.recommendation.investment_tagline
        return final
    ...
    # --- Recommendation — the single place tagline, offer strategy, and
    # pricing classification are decided (see recommendation.py). Built
    # entirely from V2's own numbers, never mixed with V1. ---
    final.recommendation = build_recommendation(
    ...
    final.investment_tagline = final.recommendation.investment_tagline
    final.suggested_initial_offer = final.recommendation.suggested_initial_offer
    final.max_sensible_offer = final.recommendation.max_sensible_offer
    final.walk_away_price = final.recommendation.walk_away_price
    final.negotiation_reasoning = final.recommendation.offer_reasoning
