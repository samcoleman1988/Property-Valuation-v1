"""Evidence Quality validation — focus on 5 failure cases + full 20."""
...
def run_property(t):
    ...
    try:
        ...
        return {
            ...
            "groups": {g.name: {"val": g.valuation, "eq": g.evidence_quality, "wt": g.weight_in_final, "comps": g.comp_count, "fb": g.type_incompatible_fallback_count} for g in v2.groups},
        }
...
results = []
for t in PROPERTIES:
    results.append(run_property(t))

print(f"\n\n{'#' * 80}", flush=True)
print("  5-FAILURE FOCUS COMPLETE", flush=True)
print(f"{'#' * 80}", flush=True)
